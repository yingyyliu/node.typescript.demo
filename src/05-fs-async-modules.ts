...
const mergeFile = async (
  ...
  await writeFile(
    saveFilePath,
    `Result of merging text files:\r\n${contents1}\r\n\r\n${contents2}`
  );
};

console.log(`Starting first tasks...`);
readFile(filePath2)
  .then((data: string) => console.log(`File ${filePath2} contains:\r\n${data}`))
  .catch((err: any) => console.trace(`\r\n${err.stack}`));

mergeFile(saveFilePath1, filePath1, filePath2)
  .then(() => console.log(`Saved to file ${saveFilePath1}`))
  .catch((err: any) => console.trace(`\r\n${err.stack}`));
